...
84.对Web服务器进行性能评估的主要指标包括（     ）

A.丢包率❎ B.最大并发连接数 C.响应延迟 D.吞吐量

> Web服务器性能评估关注的是“能同时服务多少用户、多快返回结果、单位时间能处理多少请求”，与网络层丢包无关

- 丢包率——属于网络链路质量指标，不是 Web 服务器本身的性能度量

85.区块链技术的特点是（  **ABCD**   ）

A.去中心化 B.分布式 C.共识机制✅ D.不可篡改
...
B.敏捷方法强调软件过程与工具胜过个体和交互 ❎ （反了）
C.敏捷方法强调尽早提交有价值的软件 ✅
D.敏捷方法强调小版本发布✅
```

- 个体和交互 胜过 过程与工具；可工作的软件 胜过 面面俱到的文档；客户合作 胜过 合同谈判；响应变化 胜过 遵循计划
- 要“尽早地、持续地交付有价值的软件”，且推荐“小版本、可运行”频繁发布
...
A. 扩展了通信范围 B. 减少了网络的流量✅ C. 对用户可以分别实施管理✅  D. 提高了网络的利用率✅

- VLAN 实际上是逻辑上划分网络，并不是扩展物理通信范围

93.某地教育部门的高考查分系统遭遇数据库性能瓶颈，以下哪些方式能 **短时间内** 针对性地解决问题？
...
23.通常在信息系统配置中采用容错技术，提供相应的可靠性，( )是集群的一种形式，两台服务器均处于活动状态，同运行相同的应用，以保证整体系统的性能也实现了负载均衡和互为备份。

A.**双机双工模式**✅ B.双机热备模式 C.双机集群模式 D.双机互备模式

> 两台机器同时工作、分担负荷，任何一台故障时另一台接管其全部负载，既做到高可用又实现负载均衡

26.企业资源规划(ERP)中的企业资源包括企业的“三流”资源,不包含以下

A.**数据流** B.物流 C.资金流 D.信息流

- 三流: 物流（物的流动）、资金流（钱的结算）、信息流（计划、单据、报表的传递）

28.逆向工具属于软件过程中 **软件维护** 的工具

- 软件过程活动主要分为：^^开发、维护、管理、支持^^
    - 开发：包括需求、设计、编码、测试等正向过程
    - 维护：包括修正性、适应性、完善性、预防性维护，逆向工程是维护阶段理解软件的重要工具
    - 管理：项目管理、配置管理
    - 支持：文档工具、配置管理工具

30.环形复杂度公认的可维护性上限是 **10**

32.某模块实现两个功能:向某个数据结构区域写数据和从该区域读数据。该模块的内聚类型为(  )内聚

A.过程 B.通信✅ C.逻辑 D.时间

> 先写后读，只是“对同一数据区通信”，并未按流程顺序绑定（非过程），也不是按时间先后触发（非时间），更不是按逻辑分类（非逻辑）

33.在某销售系统中，客户采用扫描二维码进行支付。若采用面向对象方法开发该销售系统，则二维码类属于( )类

A.接口 B.实体 C. 控制 D.状态

在面向对象分析/设计中，常用 ^^实体类、控制类、边界类（ECB）^^ ：

- 边界类（接口类）：系统与外部参与者交互的类，如UI、通信接口、传感器接口
- 控制类：负责业务逻辑或流程控制
- 实体类：代表业务域中的关键概念或数据存储对象

> 二维码在这里是系统与外部（用户支付）交互的媒介，即A

36.配置管理属于信息系统软件运维体系中的（）

A.运维支持要素 B.运维管理过程✅ C.运维需求 D.运维管理原则

- A：指人员、技术、资源等支撑条件
- C: 指功能需求、性能需求等

41.某公司承接了某海外信息系统集成项目，项目进行中，项目经理获悉因天气和汇率原因，预计设备到场的运费比预算高出30%，接下来他应该首先

A.项目还没有结束，暂时不做处理 B.给主管领导打电话，寻求解决方案 C.**填写变更申请,启动变更流程**✅ D.寻找新的承运商，评估变更影响，提交合同变更申请

- 先记录并启动正式变更 → 填写变更申请（C）；随后再评估替代方案、影响、寻找新承运商（D）供 CCB 审批
- B跳过流程，不合规

42.在关键路径上增加资源不一定会缩短项目的工期，这是因为：

```
A.关键路径上的活动是不依赖于时间和资源的
B.增加资源有可能导致产生额外的问题并且降低效率 ✅
C.关键活动的历时是固定不变的
D.关键活动所配置的资源数量总是充足的
```

46.以下()属于数据挖掘中 **关联分析算法**

A.K-means B.EM C.Apriori✅ D.SVM

- 关联=Association => C
- K-means、EM是聚类，SVM是分类/回归

50.数据模型三要素：^^**数据结构、数据操作和完整性约束条件**^^

52.“把对表E的属性Salary的修改权授予用户WU”的SQL语句是：
```sql
GRANT 权限(列名) ON 表名 TO 用户;
GRANT UPDATE(Salary) ON TABLE E TO WU;

---删除约束
alter table drop constraint;

--- 收回用户XIAO对通讯录（TXL）中手机（mobile）的修改权
revoke update(mobile) on table TXL from XIAO;
```

53.定义学生关系模式`S(SNO，SNAME，SAGE，SDEPT，DC)`(其属性分别为学号姓名、年龄、所在系、所在系的班主任)，则该关系模式为()

A.BCNF B.3NF C.2NF D.1NF

SNO能决定其余所有属性，SNO是候选码

- **2NF：要求非主属性完全函数依赖于候选码**，`{SNAME, SAGE, SDEPT, DC}`，它们都完全依赖于整个候选码 SNO，不存在部分依赖
- **3NF：要求非主属性不传递依赖于候选码**，存在`SNO → SDEPT → DC` 不满足，故C

57.下面有关IPv6的说法正确的是

A.提供了 27 亿个地址 B.地址是层次结构的，且随机分配 C.**淘汰了广播，取而代之的是组播**✅ D.每个接口只能配置一个 IPv6 地址

- A大约340万亿亿亿个，B随机分配错，D一个接口可配置多个 IPv6 地址

66.如果张三收到李四发来的一条有 **数字签名** 的消息，那么张三应该使用（）来验证消息确实发自李四

A.张三的公钥 B.张三的私钥 C.李四的公钥 D.李四的私钥

- 私钥加密生成签名，验证肯定用公钥呀! => C
- **验证别人的签名或给别人加密，永远用对方的公钥**，自己需要解密/签名时才用自己私钥

68.某公司希望在网络上测试系统是否存在SOL注入漏洞。以下()最适合此任务

A.端口扫描器 B.网络漏洞扫描器 C.网络发现扫描器 D. **Web 漏洞扫描器**✅

- 端口扫描器：只能发现开放端口和服务，不能检测 SQL 注入
- 网络漏洞扫描器：通常扫描网络设备、操作系统漏洞，不一定专门检测 Web 应用漏洞（如 SQL 注入）
- 网络发现扫描器：用于发现网络中的活动主机和设备，与 SQL 注入无关
- Web漏洞扫描器：专门用于检测 Web 应用漏洞，包括 SQL 注入、XSS 等

68.（）可为敏感数据的保密性提供最佳保护

A.数据标签 B.数据处理 **C.数据分类**✅ D.数据消磁

70.黑客破解了连接 WiFi的 **新安装** 的婴儿监视器，对婴儿播放恐怖的声音，使孩子受到惊吓。在这个案例中，攻击者是通过()访问婴儿监视器的

A.未及时更新的恶意软件扫描程序 B.支持5GHz信道的WAP数据 C.对孩子父母实施社会工程学攻击 D.利用默认配置✅

72.数据中心多次出现硬件故障。审核员注意到服务器以密集的方式堆叠在一起没有条理。应该采取()来解决这个问题

A. 门禁控制❎ B.综合布线 C.气体灭火❎ D.热通道与冷通道

- BD中B，对气流有帮助，但属于辅助手段，不能根本解决热问题; D热通道与冷通道——通过封闭冷热通道、优化气流组织，显著降低机房温度，是业界针对“密集堆放”散热问题的标准做法

73.你被安排在一家跨国大型金融机构的信息安全规划团队中。你的任务是创建一个优先级列表，其中必须包含在初始设计阶段要解决的问题。所有安全工作的首要目标是()

A.预防信息泄露 B.保持完整性 C.**人身安全** ✅ D.维持可用性

- CIA(机密性、完整性、可用性)是在人身安全之后的安全目标

74.组合策略的备份还原速度最快：

A.**完整备份和差异备份**✅ B.部分备份和增量备份 C.完整备份和增量备份 D.增量备份和差异备份

- 增量备份需要“完整 + 所有增量链”，逐卷回拷最耗时

77.商业银行应制定相关控制信息系统变更的制度和流程，其中应包括生产系统与开发系统、测试系统()

A.物理隔离 B.**有效隔离**✅ C.逻辑隔离 D.空间隔离 ~~(B读起来最顺)~~

82.从技术的角度来看，影响软件可靠性的主要因素包括：

A.运行剖面(环境)✅ B.软件规模✅ C.软件内部结构✅ D.软件的开发方法和开发环境✅

- A：软件在不同使用环境、输入组合下的运行情况直接影响其可靠性表现
- D: 开发方法（如敏捷、形式化方法）和工具环境对软件质量有显著影响
...
C.在团队内部，最具有效果并富有效率的信息传递方法是面对面的交谈✅ （漏选）
D.强调应对需求的持续变更，即使在项目后期也可灵活应对需求变更✅
```

84.工作量估算模型COCOMOI的层次结构中，估算选择主要包括

A.对象点✅ B.功能点✅ C.用例数 D.源代码行✅

- 应用组合模型（早期）——采用 对象点
- 早期设计模型——使用 功能点再转换为 源代码行
- 后架构模型——直接基于 源代码行 进行详细估算

85.为解决组织中多个项目之间的资源冲突问题，可以采用的方法:

```
A.制定资源计划时，每个项目预留尽量多的资源富余量 ❎
    - 导致整体资源利用率下降，反而加剧资源紧张
B.检查组织内部的资源使用情况，看是否有资源分配不合理的情况✅
C.制定资源在项目间分配的原则，重要的项目优先得到资源✅
D.将组织中的资源进行统一管理，避免资源浪费和过度使用 ✅
```

软件变更控制对保障软件项目开发质量特别重要，其中( **提取** )过程实现了变化控制的两个主要功能:访问控制和同步控制

88.关于OLAP和OLTP的叙述中正确的有

```
A.OLTP事务量大，但事务内容比较简单且重复率高 ✅
B.OLAP的最终数据来源与OLTP不一样 ❎
    - OLAP的数据通常来源于OLTP系统
    - 经过 ETL(提取、转换、加载) 到数据仓库
    - 原始数据来源相同
C.OLAP 面对决策高层人员 ✅
D.OLTP 以应用为核心，是应用驱动的 ✅
    - OLTP 围绕具体业务应用展开，支持日常操作
```


90.对数据库进行 **安全性控制**，防范的对象有:

A.非法用户✅ B.非法操作✅ C.不合语义的数据❎ D.“脏”数据❎

- C数据完整性或业务规则约束范畴,D并发控制问题

91.下列关于数据库中冗余数据的说法，正确的有

```
A.数据库中的冗余数据越少越好 ❎
    - 有时为了性能（如反规范化）会适当保留冗余数据
B.冗余数据不会破坏数据库的完整性 ❎
    - 冗余数据如果更新不同步，会导致数据不一致，破坏完整性
C.冗余数据的存在给数据库的维护增加了难度 ✅
D,数据库中有必要存储一定的冗余数据 ✅
```

92.网络管理员监测到局域网内计算机的传输速度变得很慢,可能的原因有:

```
A.WannaCry勒索病毒 ✅ 
    - （蠕虫传播特性，产生大量网络扫描流量，导致 LAN 堵塞、速度变慢）
B.网络广播风暴 ✅
D.运营商互联网接人故障 ✅
    - 如网线损坏、接口氧化等会导致信号质量差、丢包重传，有效传输速率降低
C.网络线路介质故障
```

95.某机构运营着一个公司网站,目前使用的是一台服务器，能够处理网站的全部负载。能够提高该服务器运行可靠性的有

```
A.在服务器中安装双电源 ✅
B.在负载均衡器后面部署多台服务器
    - 题干是提升该服务器，用负载均衡整体可靠性提升了
    - 但这台服务器没啥变化
C.用RAID阵列替换服务器的硬盘。
D.定期备份服务器
    - 虽然备份不能让服务器不宕机
